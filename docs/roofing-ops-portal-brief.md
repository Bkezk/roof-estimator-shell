...
| **Inventory**                                          | One screen for the crew (owner, Sep 24): stock on hand at the shop and on each service vehicle (`inventory_locations`; every entry says where), with "Take from inventory" and "Put in inventory" on every row — the form asks WHERE first, then the product, amount and job; "Load a vehicle" / "Return to shop" move stock between the shop and a vehicle and write off what did not come back as used on the vehicle; jobs draw from the shop only, a search box for the product (name or item #) and one for the job that remembers the last job used; the entry carries who and when and shows on the bid. Inventory-only logins record leftovers and job use; adjustments and write-offs stay with estimators. Pieces or packs, true units (tube, bag, cartridge, roll); history with reasons; settings. On a bid's Review step an **Order list** shows every product the bid needs, what is on hand, "Use from inventory" / "Put back" per line, and **To buy** = needed − what this bid actually pulled. Print/PDF and Excel export. Stock never changes bid price | `/inventory`, Review step                            |
...
| County PVA parcels            | Owner of record and mailing address, property address (where published), property class, lot area, deed book-page (a change = a transfer) | One ArcGIS MapServer per county at `kygisserver.ky.gov/arcgis/rest/services/WGS84WM_Services/Ky_PVA_<County>_Parcels_WGS84WM` — verified Sep 24 on Webster County (Web Mercator shapes, `AREA` sq ft, `PERIMETER` ft; field names vary by county, mapped from the layer's field list). Parcels are LAND; roof area needs footprints | Free                                                   | 1 (built: Preview / Import on the Buildings page)         |
