...
**Names are keys.** (Accepted aliases, from the template build on Sep 25: `Membrane Color` for
`Color`, `Curb Type` for `Type` on a curb, `Scupper Type` for `Type` on a scupper,
`Edge Perimeter` for `Edge: Perimeter`, `Item Name` for `Section Name` / `Wall Name` /
`Curb Name`, and underlayment names written `4x4` for `4'x 4'` — the importer strips foot marks
and spaces before comparing board names. A `BOM Roof Section` may carry its own `Roof System`,
`Membrane Mil`, `Color`, `Sheet Size`, `Field Tab Spacing` and `Deck Type`; when present they
win over the job setup for that section, which is how the estimator treats a section anyway.) Bid-O-Matic matches PlanSwift items and property values by exact text
...
`Field Fastener OC` must come from the pull-test lookup (Part C §C.6), not a fixed default: for
the estimator's own defaults (Duro-Last, 60 psf, 350 lbs, 60" tab) the lookup gives **15 in**,
and 12 in would overstate screws and labor by a quarter.
...
| Wood blocking                | The estimator's automatic rows are the first four of `non_dl:roof_edge_blocking` (`½"`, `¾"`, `5/4"`, `2" Wood Blocking`), priced **per linear foot** per board thickness stacked to the underlayment thickness, quantity `RoundUp(boards × ft × 1.03)`; their prices are **$0 in the catalog today** (never captured), so both PlanSwift and the estimator bill $0 material for them until the owner enters per-foot prices on Estimate Pricing › Non-DL › Roof Edge Blocking. The lumber rows (`2" x 6" x 12'` and so on) are **per 12 ft piece**: pieces = `RoundUp(ft / 12)`, labor `LaborPerUnit` per piece. | as stated per row                                               |
