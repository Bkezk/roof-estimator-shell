...
export function parapetEdgeFasteners(
  ...
  let total = 0;
  {
    ...
    if (roofSystem === "Duro-Last") {
      ...
      roofSystem === "Non-DL TPO" ||
      roofSystem === "EPDM Rubber"
    ) {
      // The web-only systems (§22.34 / §22.35) borrow the Duro-Tuff wall-tab rule — flagged.
      total +=
...
export function computeAccessories(args: ComputeAccessoriesArgs): AccessoriesResult {
  ...
  const rowStyle =
    ...
    ["Duro-Last", "Duro-Roof", "Duro-Tuff", "Duro-Tech TPO", "Non-DL TPO", "EPDM Rubber"].includes(
      args.roofSystem,
    );
  for (const s of args.sections) {
